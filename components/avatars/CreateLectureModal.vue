      ...
      <div class="modal-body">
        ...
        <div class="field">
          ...
          <div v-else class="audio-ready">
            <span class="file-type-tag">{{ file.name.endsWith('.pdf') ? 'PDF' : 'PPTX' }}</span>
        ...
        <div v-if="uploading" class="upload-progress">
          <div class="upload-bar-wrap"><div class="upload-bar" :style="{ transform: `scaleX(${uploadPct / 100})` }"></div></div>
...
.ico-lec { background: rgba(var(--teal-rgb),.12); color: var(--teal); border: 1px solid rgba(var(--teal-rgb),.2); }
...
.file-type-tag{display:inline-flex;align-items:center;justify-content:center;background:var(--teal);color:#fff;font-size:9px;font-weight:800;letter-spacing:.04em;padding:3px 7px;border-radius:4px;flex-shrink:0}
...
.dur-chip.active { background: rgba(var(--teal-rgb),.1); color: var(--teal); border-color: rgba(var(--teal-rgb),.35); }
.dur-chip:hover { border-color: rgba(var(--teal-rgb),.3); }
...
.style-chip.active { background: rgba(var(--teal-rgb),.08); border-color: rgba(var(--teal-rgb),.35); }
