...
const { avatarRegistry } = useUserRegistries()
...
const chatAvatar=computed(()=>{
  ...
  return avatarRegistry.value[other.id]||''
})
...
.cli.active{background:rgba(var(--teal-rgb),.1);border:1px solid rgba(var(--teal-rgb),.14)}
...
.cli-badge{background:linear-gradient(135deg,var(--teal),var(--teal-h));color:#fff;font-size:11px;font-weight:700;padding:2px 7px;border-radius:100px;flex-shrink:0;min-width:20px;text-align:center}
