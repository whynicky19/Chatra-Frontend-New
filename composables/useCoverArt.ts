...
export const COVER_ICON_PATHS: Record<string, string> = {
  ...
  ruler: 'M3 14.5l11.5-11.5 6.5 6.5L9.5 21M7 11l1.5 1.5M10 8l1.5 1.5M13 11l1.5 1.5M10 14l1.5 1.5M13 17l1.5 1.5',
  ai: 'M9 4.5h6M10 4.5v-1.5h4v1.5M10 4.5v3l-4.5 6.5a1.5 1.5 0 0 0 1.3 2.3h10.4a1.5 1.5 0 0 0 1.3-2.3L14 7.5v-3M7 17.5a1.5 1.5 0 0 1 3 0 1.5 1.5 0 0 1-3 0M14 17.5a1.5 1.5 0 0 1 3 0 1.5 1.5 0 0 1-3 0',
  mobile: 'M7 3.5h10v17H7zM7 6.5h10M7 17.5h10M10 19.5h4',
  cloud: 'M7 18.5a4.5 4.5 0 0 1-.6-8.9A6.5 6.5 0 0 1 19.4 11a4 4 0 0 1-1 7.5H7zM9 13.5l3-3M12 10.5v6M9 16.5l3-3',
  ...
  translation: 'M3 5h8M7 3v2M4.5 5c0 4 3 7 6.5 8M9 9c-.5 2-2 3.5-3.5 4M12 21l3-7 3 7M14 19h4M16.5 11l1.5 3M11 12c0 3 2 5 4 6',
  ...
  calculator: 'M5 4.5h14v15H5zM8 8h8M8 12.5h2M12 12.5h2M16 12.5h0M8 16h2M12 16h2M16 16h0M14 16h2',
  bank: 'M3 10.5h18M5 10.5v9M9 10.5v9M15 10.5v9M19 10.5v9M3 19.5h18M12 3.5 21 9.5H3z',
...
export const COVER_ICON_LABELS: Record<string, Record<string, string>> = {
  ...
  dice:        { ru: 'Теория вероятностей', en: 'Probability',   kk: 'Ықтималдық теориясы' },
  algebra:     { ru: 'Алгебра',         en: 'Algebra',          kk: 'Алгебра' },
  prob:        { ru: 'Статистика',      en: 'Statistics',       kk: 'Статистика' },
  ...
  anatomy:     { ru: 'Анатомия',        en: 'Anatomy',          kk: 'Анатомия' },
  ...
  lock:        { ru: 'Информационная безопасность', en: 'Info Security', kk: 'Ақпараттық қауіпсіздік' },
  ...
  ruler:       { ru: 'Черчение',        en: 'Drafting',         kk: 'Сызу' },
  ai:          { ru: 'ИИ и машинное обучение', en: 'AI & Machine Learning', kk: 'ЖИ және машиналық оқыту' },
  mobile:      { ru: 'Мобильная разработка', en: 'Mobile Development', kk: 'Мобильді әзірлеу' },
  cloud:       { ru: 'Облачные технологии', en: 'Cloud Computing', kk: 'Бұлттық технологиялар' },
  terminal:    { ru: 'Программирование (CLI)', en: 'Scripting & DevOps', kk: 'Скрипт жазу' },
  ...
  podium:      { ru: 'Право и политика', en: 'Law & Politics',  kk: 'Құқық және саясат' },
  gavel:       { ru: 'Суд и право',     en: 'Law & Courts',     kk: 'Сот және құқық' },

  // ── Языки ─────────────────────────────────────────────────────────────
  letter:      { ru: 'Иностранный язык', en: 'Foreign Language', kk: 'Шет тілі' },
  chat:        { ru: 'Речь и общение',  en: 'Communication',    kk: 'Сөйлеу мәдениеті' },
  flag:        { ru: 'Родной язык',     en: 'Native Language',  kk: 'Ана тілі' },
  translation: { ru: 'Перевод',         en: 'Translation',      kk: 'Аударма' },
  ...
  trending:    { ru: 'Маркетинг и рост', en: 'Marketing & Growth', kk: 'Маркетинг' },
  calculator:  { ru: 'Бухгалтерия',     en: 'Accounting',       kk: 'Бухгалтерия' },
  bank:        { ru: 'Банковское дело', en: 'Banking',          kk: 'Банк ісі' },
...
export const COVER_GROUP_LABELS: Record<string, Record<string, string>> = {
  ...
  tech:       { ru: 'IT и инженерия',     en: 'IT & engineering', kk: 'IT және инженерия' },
  ...
  health:     { ru: 'Медицина и здоровье', en: 'Health & medicine', kk: 'Медицина және денсаулық' },
}
  ...
  const load = async (): Promise<CoverOptions> => {
    ...
    inflight = api.get('/classes/cover/options')
      .then(({ data }) => {
        ...
        if (!(o?.colors?.length && o?.icons?.length)) {
          cached.value = FALLBACK_COVER_OPTIONS
          return cached.value
        }
        // Отфильтровать иконки, для которых у фронта нет глифа. Бэкенд
        // может знать больше (есть легаси-записи), но показывать пользователю
        // в пикере «пустую» иконку, которая при выборе превратится в book,
        // нельзя. Глифы в COVER_ICON_PATHS — источник правды для UI.
        const known = new Set(Object.keys(COVER_ICON_PATHS))
        const knownIcons = o.icons.filter((i) => known.has(i.id))
        cached.value = { ...o, icons: knownIcons.length ? knownIcons : FALLBACK_COVER_OPTIONS.icons }
        return cached.value
      })
